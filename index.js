...
function getUsersFromDatabase(array) {
  return Array.from(array, (user) => {
    ...
    return {
      id: user.id, // id який дорівнює id користувача,firstName який дорівнює firstName користувача в верхньому регістрі та years який дорівнює age користувача
      firstName: user.name.toUpperCase(),
      years: user.age, // Повертаємо масив користувачів
    };
  });
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (
    ...
    // Якщо індекси неприпустимі, виводимо рядок "Неприпустимі індекси"
...
function sortByKey(arr, key) {
  // Використовуємо метод `sort` передаємо в нього функцію порівняння для сортування масиву об'єктів за заданим ключем
...
function customAt(arr, index) {
  ...
  // Перевіряємо тип отриманого елемента та виводимо відповідне повідомлення в консоль
